...
Mambu::Mambu(glm::vec2 pos, std::vector<std::string> path, glm::vec2 speed,
             glm::vec2 ammospeed, std::string ammopath, glm::vec2 collidersize,
             int health, bool visable)
    : Enemy(pos, path[0], health, visable, collidersize,
            Enemy::HurtState::INVINCIBLE) {
    this->Path = path;
    this->Speed = speed;
    this->AmmoSpeed = ammospeed;
    this->AmmoPath = ammopath;
}
void Mambu::DoBehavior(glm::vec2 CameraPos, glm::vec2 RockmanPos,
                       int SceneStage) {
...
void Mambu::PhysicEngine() {
    if (Object->GetImagePath() == Path[0]) {
        ...
        Object->SetPosition(*Position);
        CircumlateX +=
            std::abs(Speed.x * (Util::Time::GetDeltaTimeMs() / 1000));
    }
    else {
...
void Mambu::Shoot() {
    ...
    for (int i = 0; i < 8; i++) {
        ...
        std::shared_ptr<Ammo> ammo = std::make_shared<Ammo>(
            *Position, glm::vec2{ammospeed_x, ammospeed_y}, AmmoPath,
