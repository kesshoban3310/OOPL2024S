...
void KillerBomb::DoBehavior(glm::vec2 CameraPos, glm::vec2 RockmanPos,
                            int SceneStage) {
    bool Exist =
        (6059 <= RockmanPos.x && RockmanPos.x <= 9169) && RockmanPos.y <= -740;
    bool CanMove =
        CameraPos.x - 584 <= Position->x && Position->x <= CameraPos.x + 584;
    if (Exist) {
        if (CanMove) {
            PhysicEngine();
        }
