...
Kamadoma::Kamadoma(std::vector<std::string> path,std::vector<glm::vec2> points,glm::vec2 pos,int iniidx,glm::vec2 collidersize,int hp,bool visable,Enemy::HurtState hurtstate)
...
void Kamadoma::DoBehavior(glm::vec2 CameraPos,glm::vec2 RockmanPos,int SceneStage) {
    ...
    if(RockmanPosX >= 1500) return;
    PhysicEngine();
}
void Kamadoma::PhysicEngine() {
    glm::vec2 StartPoint = Points[Idx];
    if(IsJumping){
        if(Util::Time::GetElapsedTimeMs()-JumpingTimer<50) return;
...
void Kamadoma::TrajectoryCalculator(std::pair<glm::vec2,glm::vec2> point){
    ...
    JumpingTime = std::sqrt(2*std::abs(RealHigh-point.first.y)*Gravity) + std::sqrt(2*std::abs(RealHigh-point.second.y)*Gravity);
    Speed.x = (point.second.x - point.first.x) / JumpingTime;
    Speed.y = std::sqrt(2*std::abs(RealHigh-point.first.y)*Gravity) / Gravity;
}
