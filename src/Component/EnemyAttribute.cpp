...
Enemy::Enemy(glm::vec2 pos, std::string path, int hp, bool visable,
             glm::vec2 collidersize, Enemy::HurtState state,
             Enemy::LifeState lifeState, glm::vec2 scale) {
    this->Health = hp;
    this->Life = lifeState;
    this->Hurt = state;
    ...
    this->Hitbox =
        std::make_shared<Collider>(Position, collidersize, glm::vec2{0, 0});
