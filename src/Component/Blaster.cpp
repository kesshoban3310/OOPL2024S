...
Blaster::Blaster(glm::vec2 pos,unsigned long animationinterval,bool increase) {
    ...
    this->Open = increase;
    this->AmmoPath = RESOURCE_DIR "/Picture/Enemies/Bomb Man Stage/blaster/blaster5.png";
    Initialize();
}
...
void Blaster::Behavior(glm::vec2 pos) {
    if(Open) {
        if (Util::Time::GetElapsedTimeMs() - AnimationTimer > AnimationInterval) {
            if(PathIndex<3)
...
void Blaster::Shoot() {
    ...
    std::string path = RESOURCE_DIR "/Picture/Enemies/Bomb Man Stage/blaster/blaster5.png";
    glm::vec2 ammosize = {8*3,8*3};
    std::shared_ptr<Ammo> ammo = std::make_shared<Ammo>(glm::vec2 {position->x+24,position->y},ammospeed,path,ammosize);
